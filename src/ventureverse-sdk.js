/**
 * VentureVerse SDK for Third-Party App Integration
 * 
 * This SDK simplifies integration with the VentureVerse platform,
 * handling user authentication, credit management, and secure communication.
 * 
 * Usage:
 * ```javascript
 ...
 *   appId: 'your-app-id',
 *   debug: true,
 *   encryptionKey: 'your-encryption-key' // For secure parameter handling
 * });
 * 
...
export class VentureVerseSDK {
  constructor(options = {}) {
    ...
    this.log('SDK initialized with options:', { ...this.options, encryptionKey: '[HIDDEN]' });
  }
   ...
   */
  async requestPermission(action, appName = 'This app') {
    if (!this.isIframeMode) {
      return confirm(`Demo: ${appName} wants to ${action}. Allow?`);
    }
  ...
  createSecureIframeUrl(baseUrl, userContext) {
    ...
    try {
      ...
      if (this.options.appId && this.options.appId !== 'unknown') {
  ...
  log(...args) {
    if (this.options.debug) {
      console.log('[VentureVerse SDK]', ...args);
    }
  }
